...
export default function TrafficStats() {
  ...
  const exportToExcel = () => {
    ...
    link.download = `流量统计_${new Date().toLocaleDateString()}.csv`;
    link.click();
  };
  ...
  const TagCard = ({ title, value, tag, icon: Icon, color }: { title: string; value: number; tag?: string; icon: React.ElementType; color: string }) => (
    ...
    >
      ...
      <div className="flex items-center justify-between text-sm mt-1">
        <span className="text-gray-500">较前日同时段</span>
        <span className="text-gray-400">-</span>
      </div>
    </motion.div>
  );
        ...
        <div className="p-6">
          ...
          >
            ...
            <div className="h-64 flex items-end justify-between px-4">
              {hourlyData.length === 0 ? (
                <div className="w-full text-center text-gray-400 py-8">暂无数据</div>
              ) : (
                (() => {
